...
| LLM | キーワード駆動モック | **Gemini (Google AI Studio API キー)** |
...
| LLM | **Gemini** via `langchain-google-genai`（Google AI Studio API キー） |
...
- **RLS**: 全テーブルで `user_id = auth.uid()` ベースのポリシー。ログ等の共有範囲は要確認（§10）。
...
- **Gemini モデル名**: 既定値と切替方針（`GEMINI_MODEL` 環境変数で設定。v1.0 placeholder は `gemini-3.1-pro-preview` 系）。
- **ログ共有範囲**: `tool_call_logs` / セッションを「全社員に公開」(v1.0 前提) のままにするか、RLS でユーザー/部署単位に絞るか。
- **カタログモデルの初期ラインナップ**: どの SD モデルを同梱するか（売上・在庫・人員 等）。
